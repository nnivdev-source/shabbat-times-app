...
const showToast = (msg) => { const t = $('toast'); if (t) { t.textContent = msg; t.style.opacity = '1'; setTimeout(() => t.style.opacity = '0', 3000); } };
...
async function fetchExternalCities() {
  try {
    const resp = await fetch('https://data.gov.il/api/action/datastore_search?resource_id=b7cf8f14-64a2-4b33-8d4b-edb286fdbd37&limit=1500');
    const data = await resp.json();
    if (data.success) {
      externalCities = data.result.records.map(r => ({
        n: r['שם_ישוב'].trim(),
        d: r['שם_ישוב_לועזי'].trim(),
        c: 'ישראל',
        isExternal: true
      })).sort((a, b) => a.n.localeCompare(b.n, 'he'));
      renderCityList('');
    }
...
async function fetchShabbatTimes(city, customName = null) {
  ...
  try {
    let url;
    if (city.geo) url = `https://www.hebcal.com/shabbat?cfg=json&geonameid=${city.geo}&m=${candleMinutes}&M=on&lg=he`;
    else if (city.lat) url = `https://www.hebcal.com/shabbat?cfg=json&latitude=${city.lat}&longitude=${city.lng}&tzid=Asia/Jerusalem&m=${candleMinutes}&M=on&lg=he`;
    else url = `https://www.hebcal.com/shabbat?cfg=json&city=${encodeURIComponent(city.n)}&m=${candleMinutes}&M=on&lg=he`;
...
function parseData(data) {
  ...
  if (p) { state.parasha = (p.hebrew || p.title).replace('Parashat ', '').replace('פרשת ', ''); state.hdate = p.hdate; }
  ...
  const findZman = (keys) => { const item = items.find(i => keys.some(k => i.title && i.title.includes(k))); return item ? item.date : null; };
  state.allZmanim = [
...
function render() {
  ...
  setVal('loc-name', currentCity.n);
  setVal('loc-sub', currentCity.c);
  ...
  if (zList) zList.innerHTML = state.allZmanim.map(z => `<div class="zmanim-item"><span class="zmanim-label">${z.label}</span><span class="zmanim-value">${formatTime(z.val, tz)}</span></div>`).join('');
  if (state.parasha) {
    ...
    } else { if ($('parasha-pts')) $('parasha-pts').innerHTML = '<div class="inspiration-point">שבת שלום ומבורך!</div>'; if ($('parasha-pasuk')) $('parasha-pasuk').textContent = ""; }
  }
  updateCountdown();
...
function updateCountdown() {
  ...
  if (diff < 0) { $('countdown').innerHTML = new Date() < new Date(state.havdala) ? 'שבת שלום! <span>שבת כבר נכנסה</span>' : 'שבוע טוב! <span>השבת הסתיימה</span>'; return; }
...
function formatGregorian(iso) {
  if (!iso) return '';
  const d = new Date(iso), M = ['ינואר', 'פברואר', 'מרץ', 'אפריל', 'מאי', 'יוני', 'יולי', 'אוגוסט', 'ספטמבר', 'אוקטובר', 'נובמבר', 'דצמבר'], D = ['ראשון', 'שני', 'שלישי', 'רביעי', 'חמישי', 'שישי', 'שבת'];
...
function renderCityList(filter) {
  ...
  if (!container) return;
  const merged = [...CITIES, ...externalCities];
  const list = filter ? merged.filter(c => c.n.includes(filter) || c.d.toLowerCase().includes(filter.toLowerCase())) : merged.slice(0, 50);
...
window.setCity = (json) => { const city = JSON.parse(decodeURIComponent(json)); $('city-panel').style.display = 'none'; fetchShabbatTimes(city); };
...
const initApp = () => {
  ...
  if (!localStorage.getItem('shabbat_consent') && modal) modal.style.display = 'flex';
...
if ($('btn-calendar')) $('btn-calendar').onclick = () => {
  ...
  const start = new Date(state.candle).toISOString().replace(/-|:|\.\d+/g, ""), end = new Date(new Date(state.candle).getTime() + 60 * 60000).toISOString().replace(/-|:|\.\d+/g, "");
...
document.querySelectorAll('.bless-tab').forEach(tab => tab.onclick = () => {
  ...
  tab.classList.add('active'); state.currentBlessing = tab.dataset.tab; renderBlessing(state.currentBlessing);
});
if ($('btn-share-main')) $('btn-share-main').onclick = () => {
  ...
  if (navigator.share) navigator.share({ title: 'זמני השבת', text }); else { navigator.clipboard.writeText(text); showToast('הזמנים הועתקו'); }
};
...
if ($('btn-notify')) {
  $('btn-notify').onclick = async () => {
    ...
    if (await Notification.requestPermission() === 'granted') { showToast('התראות הופעלו!'); new Notification('נרות שבת', { body: 'נזכיר לך להדליק נרות בזמן.', icon: '/favicon.ico' }); } else showToast('יש לאשר התראות');
  };
}
